...
I used to think that better ML meant using a bigger model or fine-tuning more parameters. Working on LoRA and knowledge distillation changed that. I saw that smaller, more efficient models could retain much of the performance while being cheaper and easier to run. It made me care more about the tradeoff, not just the score.
...
At Check Point, I started digging beyond my immediate task when I noticed debugging AI services meant tracing problems across models, tools, streaming, and APIs. I ended up working on the model proxy and MCP integrations and testing the pieces end to end. I was driven by wanting to understand where things actually broke, which made debugging much faster.
...
At Check Point, I initially treated model choice as mostly a question of which model gave the best answer. After working with different providers and tasks, I noticed that the “best” model depended heavily on the job. I started routing requests based on the task instead of sending everything to one model. That made workflows more reliable and avoided unnecessary cost.
